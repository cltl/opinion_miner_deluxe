...
def link_exp_tar(expressions,targets, kaf_obj):
    ...
    if len(targets) == 0:
        ...
        for exp_ids in expressions:
            for tar_ids in targets:
                feats_exp, feats_tar,extra_feats_exp, extra_feats_tar = extract_feats_from_spans_kaf(exp_ids,tar_ids,kaf_obj)
...
def link_exp_hol(expressions,holders, kaf_obj):
    ...
    if len(holders) == 0:
        ...
        for exp_ids in expressions:
            for hol_ids in holders:
                feats_exp, feats_hol,extra_feats_exp, extra_feats_hol = extract_feats_from_spans_kaf_exp_hol(exp_ids,hol_ids,kaf_obj)
...
def link_entities_svm(expressions, targets, holders, kaf_obj,this_config_manager):
    all_types = []
    all_exp_ids = []
    all_tar_ids = []
    all_hol_ids = []
    ...
    for exp_ids,exp_type in expressions:
        ...
        exp_term_ids = map_tokens_to_terms(exp_ids, kaf_obj)
        all_exp_ids.append(exp_term_ids)
    
    for tar_ids, tar_type in targets:
        tar_term_ids = map_tokens_to_terms(tar_ids, kaf_obj)
        all_tar_ids.append(tar_term_ids)

    for hol_ids, hol_type in holders:
        hol_term_ids = map_tokens_to_terms(hol_ids, kaf_obj)
        all_hol_ids.append(hol_term_ids)
    
    assigned_targets = link_exp_tar(all_exp_ids, all_tar_ids,kaf_obj)
    assigned_holders = link_exp_hol(all_exp_ids, all_hol_ids, kaf_obj)
